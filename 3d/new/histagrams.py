...
def plot_hista_par_d(paricles):
    particles_distances = []
    for i, p1 in enumerate(particles):
        for p2 in particles[i + 1:]:
            particles_distances.append(abs(p1.r - p2.r))

    plt.hist(particles_distances, bins='auto')
    plt.show()
...
sorted_particles = sorted(particles, key=lambda particle: abs(particle.r))
particles_distances = []
# for i, p1 in enumerate(particles):
#     for p2 in particles[i+1:]:
#         particles_distances.append(abs(p1.r-p2.r))
p2 = sorted_particles[0]
for p1 in sorted_particles:
    particles_distances.append(abs(p1.r-p2.r))

fig = plt.figure(figsize=(15, 15))
ax = fig.add_subplot(1, 1, 1)
plt.xticks(np.linspace(0, 4, 50))
ax.hist(particles_distances, bins='auto')
# plt.hist(particles_distances, bins=1956)
