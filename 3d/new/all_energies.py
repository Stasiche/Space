...
def get_and_plot_density(particles, radii, num=100):
    ...
    lengths_tmp = sorted([abs(particle.r) for particle in particles])
    density = []
    l = []
    ...
    for i in range(len(lengths)//num):
        ...
        min_l = 10*constants.a_0
        max_l = 0
        mean_l = 0
        k = 0
        for l1 in lengths_tmp[i*num:i*num + num-1]:
            for l2 in lengths_tmp[i*num:i*num + num-1]:
                if l1 != l2:
                    if abs(l1-l2)/constants.a_0 < min_l:
                        min_l = abs(l1-l2)/constants.a_0
                    if abs(l1-l2)/constants.a_0 > max_l:
                        max_l = abs(l1-l2)/constants.a_0
                    mean_l += abs(l1-l2)/constants.a_0
                    k += 1
        mean_l /= k
        print("form %f to %f" % (lengths_tmp[i*num], lengths_tmp[i*num + num-1]))
        print("min :", min_l)
        print("mean :", mean_l)
        print("max :", max_l)
        print('______')
...
save_path = os.path.dirname(os.path.realpath(__file__)) + '/test_all_energies_2000'
