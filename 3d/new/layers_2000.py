...
def get_and_plot_density(particles, radii, rot_axis, num=100):
    ...
    l.append(lengths[-1])
    return l, av_random_v_m_list, av_omega_list
...
def create_sphere4(radii, info, rot_axis, num_sumples=2000, mass=1):
    particles = []
    l, av_random_v_m_list, av_omega_list = info
    for i, el in enumerate(l[:-1]):
        cur_num_particles = 0
        r_min = el
        r_max = l[i+1]
        av_v_m = av_random_v_m_list[i]
        ...
        while True:
            ...
            if (cur_num_particles == num_sumples//len(av_random_v_m_list)) or\
                    ((cur_num_particles == num_sumples % len(av_random_v_m_list)) and (i == len(av_random_v_m_list)-1)): ##!!!!
                break
...
save_path = os.path.dirname(os.path.realpath(__file__)) + '/layers_2000'
...
#            max_r = r
#        mean_r += r
#print(min_r, mean_r/counter, max_r)
#
#tmp_l = [0 for i in range(batch_num)]
#for p in rs:
#    ind = int(np.floor(((p-min_r)/max_r)*batch_num))
#    if ind == batch_num:
#        ind -= 1
#    tmp_l[ind] += 1/counter
#
#for i, el in enumerate(tmp_l):
#    tmp_l[i] = np.round(el, 4)
#print(tmp_l)
...
info = get_and_plot_density(particles, radii, rot_axis)
