    ...
    for i, el in enumerate(l[:-1]):
        ...
        while True:
            ...
            if (cur_num_particles == num_sumples // len(av_random_v_m_list)) or \
                    ((cur_num_particles == num_sumples % len(av_random_v_m_list)) and (
                            i == len(av_random_v_m_list) - 1)):  ##!!!!
                break
...
save_path = os.path.dirname(os.path.realpath(__file__)) + '/lot_layers_2_2000'
...
info = get_and_plot_density(particles, radii, rot_axis, 2)
print("nl", len(info[1]))
